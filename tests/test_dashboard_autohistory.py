...
def test_auto_history_includes_in_progress_current(tmp_audit, sample_findings, sample_profile):
    """The audit being rendered is still in-progress; it must appear in its own trend."""
    for ts_id in ("202601011000", "202602011000"):
        write_audit(tmp_audit / ts_id, sample_findings, profile=sample_profile)
    h = {"repo": "r", "remote": "", "audits": [
        {"id": "202601011000", "dir": "202601011000", "status": "complete",
         "timestamp": "2026-01-01T10:00:00Z", "commit": "", "branch": "",
         "tier": "", "baseline_from": None},
    ...
    history = render_dashboard.build_history_from_manifest(tmp_audit, "202602011000")
    assert [x["label"] for x in history] == ["202601011000", "202602011000"]
